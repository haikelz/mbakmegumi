...
function createTouchTexture() {
  ...
  function addTouch(norm: { x: number; y: number }) {
    let force = 0;
    let vx = 0;
    let vy = 0;
    if (last) {
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        ...
        t.composer?.dispose();
        t.renderer.dispose();
      if (t.renderer.domElement.parentElement === host)
        host.removeChild(t.renderer.domElement);
      ...
      const uniforms = {
        ...
        uClickPos: {
          value: Array.from({ length: MAX_CLICKS }, function () {
      ...
      function mapToPixels(e: PointerEvent) {
